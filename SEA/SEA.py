...
for i in range(file_num):
    if(label[i] == 1):
        ...
        cnt_ = cnt_ + 1
# cnt, cnt_
a = 1 - (car_D.mean() - car_D.min()) / (car_D.max() - car_D.min())
b = 1 - (back_D.mean() - back_D.min()) / (back_D.max() - back_D.min())
SEA_D = (a + b) / 2
print('SEA_D:', SEA_D)
...
for i in range(file_num):
    if(label[i] == 1):
        ...
        cnt_ = cnt_ + 1
# print(cnt, cnt_)
a = ((car_B.mean() - car_B.min())/(car_B.max() - car_B.min()))
b = ((back_B.mean() - back_B.min())/(back_B.max() - back_B.min()))
SEA_B = (a + b) / 2
print('SEA_B:',SEA_B)
